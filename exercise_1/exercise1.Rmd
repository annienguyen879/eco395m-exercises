...
fem_athletics <- subset(data, sex=="F" & sport=="Athletics", select=c(name, sex, sport, height))
...
```{r greatest variablility}
fem_events <- subset(data, sex=="F", select=c(name, sex, event, height))
fem_height_sd <- fem_events %>% group_by(event) %>% summarise(sd_height = sd(height)) %>% arrange(desc(sd_height))
```
To find which single women's event had the greatest variablility in competitor's heights (as measured by standard deviation), we first subset the data set to females. Then the data was grouped by sporting event and the standard deviation of the heights for each sport was calculated. Arranging the results in descending order, we find that Rowing Women's Coxed Fours had the greatest standard deviation in height at 10.865490.
...
```{r age Olympic swimmers}
all_swimmers <- subset(data, sport=="Swimming", select=c(name, sex, sport, age, year))
male_swimmers <- subset(data, sex=="M" & sport=="Swimming", select=c(name, sex, sport, age, year))
female_swimmers <- subset(data, sex=="F" & sport=="Swimming", select=c(name, sex, sport, age, year))
...
ggplot(swimmer_data_merge2, aes(x=year)) + 
  ggtitle("Olympic Swimming Competitors' Mean Average Age by Year: Male vs Female") +
  ...
  labs(caption=str_wrap("Average age of Olympic swimmers flucuates over time, experiencing a sharp rise and fall in the \n
                        early 1900s and then following a increasing trend thereafter. Mean male Olympic swimmers' age  \n
                        generally trends higher than the mean female Olympic swimmers' age.")) 
```
The dataset on average age of Olympic swimmers shows that women did not participate in Olympic swimming competitions from 1900 to 1948, with the exception of 1924. We see the average age of Olympic swimmers started at 18 years old in 1900 and reached a peak in 1912 at 27 years old. The data then follows a decreasing trend in the years leading up to 1960, reaching an average age of 18.65 years old in 1960. Then the data follows an increasing trend in the years that follow, with 23.24211 years old in 2016 being the most recent average Olympic swimmer's age data value available.

We see, in the data available for both genders, that male swimmers, on average, have consistently been older than female swimmers. However, in 2000, female Olympic swimmer were, on average, 22.53191 years old while male Olympic swimmers were, on average, 22.49451 years old. Hence we see a convergence in the graph of the mean average age of Olympic swimmers in the year 2000. Additionally, the average age of male Olympic swimmers experienced fluctuations in the early 1900s (where data on female Olympic swimmers was not available), notably reaching a peak in 1924 at 32 years old. However, the data on female Olympic swimmer shows a general increase in age over time.

...
```
In a single KNN regression model train/test split, we get an in sample RMSE of 58,122.03 and an out of sample RMSE of 117,355.1.

```{r knn model simulation, warning=FALSE}
knn_model = knnreg(price_train ~ lotSize + age + landValue + livingArea + bedrooms + bathrooms + rooms + newConstructionNo, data=training_set, k = 25)
